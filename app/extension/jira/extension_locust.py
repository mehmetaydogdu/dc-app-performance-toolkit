...
def fetch_static_content(locust):
    locust.get('/download/resources/io.bloompeak.jira-reports-pro:react-js/public.93e7bb93.js', catch_response=True)
    locust.get('/download/resources/io.bloompeak.jira-reports-pro:react-js/public.24dcabbe.css', catch_response=True)
    locust.get('/download/resources/io.bloompeak.jira-reports-pro:react-js/jql-editor.f9aca1bd.js', catch_response=True)
    locust.get('/download/resources/io.bloompeak.jira-reports-pro:react-js/jql-editor.6ec98ba9.css', catch_response=True)

@jira_measure("locust_app_specific_action")
# @run_as_specific_user(username='admin', password='admin')  # run as specific user
def app_specific_action(locust):
    content = locust.get('/plugins/servlet/bloompeak-jr/mainservlet/chart?dashboardId=10100&itemId=10100&isEdit=false', headers={'content-type': 'text/html;charset=UTF-8'}, catch_response=True).content.decode('utf-8')
